...
const Navbar = ({
  ...
  const mobileMenuBgColor = isMenuOpen
    ? bgColor === "bg-transparent"
      ? "rgba(0, 0, 0, 0.8)"
      : "rgba(255, 255, 255, 0.8)"
    : "transparent";
  const mobileMenuLinkColor = isMenuOpen
    ? bgColor === "bg-transparent"
      ? "text-white"
      : "text-black"
    : linkColor;
  ...
  return (
    <nav
      className={`${bgColor} ${position} top-0 left-0 w-full z-50 transition duration-300`}
    >
      <div className="container mx-auto px-4 py-5 flex justify-between items-center">
        ...
        <ul className={`hidden md:flex items-center space-x-4 ${linkColor}`}>
          <li>
            <Link
              to="/"
              className="hover:text-f1-red rounded px-3 py-2 transition duration-300"
            >
              Home
            </Link>
          </li>
        ...
        {/* Mobile Menu */}
        <div
          className={`md:hidden absolute top-16 right-0 w-full transition-all duration-300 ease-in-out ${
        ...
        >
          <ul className="flex flex-col items-end space-y-4 pr-4 pt-4">
            ...
            <li>
              <Link
                to="/"
                className={`${mobileMenuLinkColor} hover:text-f1-red`}
              >
                Home
              </Link>
            </li>
